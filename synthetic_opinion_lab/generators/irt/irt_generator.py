...
class IRTResponseGenerator(ResponseGenerator):
    ...
    def generate(self, personas: List[Persona], survey: SurveySchema) -> pd.DataFrame:
        ...
        for question in survey.questions:
            ...
            # Generate responses (add 1 to convert from 0-based IRT indexing to 1-based survey coding)
            question_responses = []
            for theta in theta_values:
                response = model.sample_response(theta) + 1
    ...
    def _get_theta_for_question(self, persona: Persona, question) -> float:
        ...
        if any(word in question_text_lower for word in ['satisfied', 'satisfaction', 'performance']):
            # Government satisfaction / trust: ideology + trust_in_government
            return 0.6 * traits.ideology + 0.4 * traits.trust_in_government

        elif 'confidence' in question_text_lower:
            # Institutional confidence: trust_in_government primary
            return 0.7 * traits.trust_in_government + 0.3 * traits.ideology

        elif any(word in question_text_lower for word in ['parliament', 'career politicians', 'lose touch', 'lie']):
            # Anti-politician populism
            return -traits.populism

        elif any(word in question_text_lower for word in ["don't have any say", "does not care", "so complicated"]):
            # External political efficacy: low trust, populist
            return -(0.5 * traits.trust_in_government + 0.3 * traits.populism)

        elif any(word in question_text_lower for word in ['immigrant', 'immigration']):
            # Immigration attitudes: cosmopolitanism (positive = pro-immigration)
            return traits.cosmopolitanism

        elif any(word in question_text_lower for word in ['traditional', 'family val', 'newer lifestyles', 'equal rights', 'women']):
            # Social conservatism: authoritarianism
            return traits.authoritarianism

        elif any(word in question_text_lower for word in ['inequality', 'gap', 'rich', 'private sector', 'jobs']):
            # Economic left-right: ideology (positive = right/low redistribution)
            return traits.ideology

        elif any(word in question_text_lower for word in ['government', 'policy', 'tax']):
            return 0.7 * traits.ideology + 0.3 * traits.trust_in_government

        elif any(word in question_text_lower for word in ['trade', 'global']):
            return traits.cosmopolitanism
