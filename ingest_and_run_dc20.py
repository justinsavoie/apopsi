#!/usr/bin/env python3
"""
Ingest DC2020, generate synthetic data with IRT/LLM/Agent, evaluate.
DC2020 variables have dc20_ prefix. 'Don't know' = 5 (recode to NaN).
"""
...
# Core variables in DC2020 (dc20_ prefix; 5 confidence items instead of 7)
CORE_VARS = [
...
FULL_TEXTS = {
    "dc20_fed_gov_satisfa":    "How satisfied are you with the performance of the federal government under Justin Trudeau?",
    "dc20_confidence_inst_1":  "Please indicate how much confidence you have in the following: The courts and justice system",
    "dc20_confidence_inst_2":  "Please indicate how much confidence you have in the following: The federal government",
    "dc20_confidence_inst_3":  "Please indicate how much confidence you have in the following: The media/news",
    "dc20_confidence_inst_4":  "Please indicate how much confidence you have in the following: Your provincial government",
    "dc20_confidence_inst_5":  "Please indicate how much confidence you have in the following: Public health officers",
    "dc20_pos_govt_care":      "The government does not care much about what people like me think.",
    "dc20_pos_govt_say":       "People like me don't have any say about what the government does.",
    "dc20_pos_govt_comp":      "Sometimes, politics and government seem so complicated that a person like me cannot really understand what is going on.",
    "dc20_pos_assim":          "Too many recent immigrants just don't want to fit in to Canadian society.",
    "dc20_pos_take_jobs":      "Immigrants take jobs away from other Canadians.",
    "dc20_pos_take_jobs_0":    "We should look after Canadians born in this country first and others second.",
    "dc20_imm_level":          "Do you think Canada should admit more immigrants, fewer immigrants, or about the same number as now?",
    "dc20_pos_family_val":     "This country would have many fewer problems if there was more emphasis on traditional family values.",
    "dc20_newerlife":          "Newer lifestyles are contributing to the breakdown of our society.",
    "dc20_inequality_gap":     "How much do you think should be done to reduce the gap between the rich and poor in Canada?",
    "dc20_pos_govt_jobs":      "The government should leave it entirely to the private sector to create jobs.",
    "dc20_party_id":           "In federal politics, do you usually think of yourself as a Liberal, Conservative, NDP, Bloc Québécois, or Green supporter?",
    "dc20_vote_choice":        "If an election were held today, which party would you be most likely to vote for?",
}


def ingest():
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    df, meta = pyreadstat.read_dta("data/Democracy Checkup 2020 v1.0.dta")
    print(f"DC2020 raw shape: {df.shape}")
    ...
    schema = SurveySchema(survey_name="DC20", questions=questions)
...
def main():
    ...
    comparison = {"survey":"DC20","model":MODEL,"timestamp":datetime.now().isoformat(),"generators":{}}
